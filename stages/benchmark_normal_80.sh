...
case "$MYSQL_VER" in
  8.0|8.4)
    ;;
  *)
    die "benchmark_normal_80.sh only supports MYSQL_VER=8.0 or 8.4"
    ;;
esac
...
if ! "$INSTALL_ROOT/bin/mysqladmin" --defaults-file="$MYSQL_ROOT_DEFAULTS_FILE" ping >/dev/null 2>&1; then
  log_warn "runtime for ${BUILD_PROFILE} is not responding; recreating it via smoke_normal_80.sh"
  env \
    MYSQL_VER="$MYSQL_VER" \
...
for db_engine in $SYSBENCH_DB_ENGINES; do
  init_log="$SYSBENCH_INIT_LOG"
  train_log="$SYSBENCH_TRAIN_LOG"
  run_phase="$BUILD_PROFILE"
  if [[ "$db_engine" != "innodb" ]]; then
    init_log="$LOG_ROOT/sysbench-init-ps-${MYSQL_VER}.${MYSQL_MINI_VER}-${BUILD_PROFILE}-${db_engine}.log"
    train_log="$LOG_ROOT/sysbench-train-ps-${MYSQL_VER}.${MYSQL_MINI_VER}-${BUILD_PROFILE}-${db_engine}.log"
    run_phase="${BUILD_PROFILE}-${db_engine}"
  fi

  log_info "initializing sysbench dataset for ${BUILD_PROFILE} db_engine=${db_engine}"
  env \
    MYSQL_ROOT_DEFAULTS_FILE="$MYSQL_ROOT_DEFAULTS_FILE" \
    MYSQL_SOCKET="$MYSQL_SOCKET" \
    SYSBENCH_BASE="$INSTALL_ROOT/sysbench" \
    dbeng="$db_engine" \
    bash "$REPO_ROOT/sysbench/init-sysbench.sh" "$INSTALL_ROOT" | tee "$init_log"
  ...
  log_info "running sysbench baseline for ${BUILD_PROFILE} db_engine=${db_engine}"
  env \
    MYSQL_ROOT_DEFAULTS_FILE="$MYSQL_ROOT_DEFAULTS_FILE" \
    MYSQL_SOCKET="$MYSQL_SOCKET" \
    SYSBENCH_BASE="$INSTALL_ROOT/sysbench" \
    dbeng="$db_engine" \
    TRAIN_MODE="$BENCHMARK_MODE" \
    RUN_PHASE="$run_phase" \
    ...
    bash "$REPO_ROOT/sysbench/train-sysbench.sh" "$INSTALL_ROOT" | tee "$train_log"
done
